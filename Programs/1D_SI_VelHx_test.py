...
## In Torrejon's inertia paper, this definition is different. 4 t_FM M_s log(2) / Delta.
...
gamma *= mu_0	# definition in the Martinez's paper.

#temperature = 300	# ambient temperature
#seed = 213	# seed of Mersenne twister

## Consider W / 1 CoFeB / 2 MgO / 1 Ta.
M_s = 7e+05	# saturation magnetization. J/Tm^3.
K_u = 4.8e+05	# magnetic anisotropy energy.
#K_eff = K_u - mu_0 * M_s**2 / 2	# effective magnetic anisotropy energy. J/m^3.
K_eff = K_u - mu_0 * M_s**2 / 2	# effective magnetic anisotropy energy. J/m^3.
...
width = 160e-09	# width of wire. 5um.
t_FM = 0.6e-09	# thickness of CoFeB. 1nm.
#D = 0.24e-03	# DMI constant. J/m^2
theta_SH = 0.07	# spin Hall angle.
P = 0	# spin polarization factor
...
current = 0.1e+12	# current density in heavy metal layer Ta / W. A/m^2.
...
# In Torrejon's inertia paper, not K_u, but K_eff is substituted.
#y_1 = odeint(one_dim_model_3var, y_0, t_1, \
#	args = (H_x, H_y, H_z, H_K(t_FM, M_s, Delta), H_D(D(current), Delta, M_s), \
#			H_R(alpha_R, P, current, M_s), H_SH(theta_SH, current, M_s, t_FM), \
#			alpha, Delta, width, Q, K_eff, M_s, A, D(current), t_FM, b_J(current, P, M_s), xi))
y_0 = y_1[-1]
...
#	args = (H_x, H_y, H_z, H_K(t_FM, M_s, Delta), H_D(D(0), Delta, M_s), \
#			H_R(alpha_R, P, 0, M_s), H_SH(theta_SH, 0, M_s, t_FM), \
#			alpha, Delta, width, Q, K_eff, M_s, A, D(0), t_FM, b_J(0, P, M_s), xi))
